...
def open_notepad_and_get_content(initial_text=""):
    """
    Opens Notepad with optional initial text, returns the content after closing.
    """
    ...
    subprocess.run(['notepad.exe', tf_name])

    with open(tf_name, 'r', encoding='utf-8') as tf:
        content = tf.read()
